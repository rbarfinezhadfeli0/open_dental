...
* **Exclude if unsent dental procedures**: Do not create statements for families with one or more family members who have unsent dental procedures, with a fee greater than $0, dated in the last 6 months. The purpose of this option is to prevent sending a statement before a claim is created (e.g. in larger organizations where billing is done in a separate department).
...
* **General Message**: Shows at the bottom of the statement in addition to dunning messages and appointment information. This text box supports [Right-Click Options](rightclicktextboxes.html).
