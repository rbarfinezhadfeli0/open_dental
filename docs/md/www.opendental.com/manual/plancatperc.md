...
For instructions on setting up an in-network plan without tracking write-offs, see [PPO Insurance Plan](planppo.html).
...
The *Out of network fee schedule treat blank entries as zero* preference determines if blank entries in the fee schedule are treated as $0 insurance estimate.
