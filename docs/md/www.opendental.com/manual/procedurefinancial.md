...
**Link Existing Adj**: Click to attach an existing adjustment to a completed procedure. Opens the Adjustment Picker window (see below). Adjustments not attached to a [Paysplit](paysplit.html), procedure, or [Payment Plan](paymentplandynamic.html) are listed. Select an adjustment and click **OK**.
