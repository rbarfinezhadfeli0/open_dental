...
Use the Check Ins Plan Fee tool to check which fee schedules are assigned to insurance plans and, if needed, quickly change the assigned fee schedule. Fee schedules can be changed for multiple plans at once. See [Check Insurance Plan Fees](feeschedulecheckfees.html) for more detailed steps and the password that is required.
