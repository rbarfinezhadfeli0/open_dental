...
| **\Sounds folder** |  |  |
| This folder is obsolete and should be deleted. All [Messaging Buttons](messagingbuttons.html) sounds are now stored in the database in sigelementdef.Sound. | -- | -- |
| **\UpdateFiles folder** |  |  |
...
| Update version upon startup. If the program version does not match pref.ProgramVersion, then Open Dental knows that it needs to update the local program. | Looks in AtoZ\UpdateFiles\Manifest.txt to decide if those files are the right version. If not, gives user option to download new Setup.exe. | Old behavior was to always download a new Setup.exe from internet. As of 12.4, it can update the local files by pulling them from the db. |
