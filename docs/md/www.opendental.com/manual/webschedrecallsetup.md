...
To begin using Web Sched Recall, first follow [eServices Signup](eservicessignup.html) for the service. Provider schedules must be created prior to utilizing Web Sched Recall (see [Schedule Setup](schedule.html)).
...
* **Max number of recalls sent every 10 minutes per clinic**: Determines how many messages are sent in a batch. Defaults to 2. Limiting the batch amount can be useful when there are many messages to be sent (hundreds) and there is concern about possible message failure.
