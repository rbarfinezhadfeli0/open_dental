...
Estimates are added as overrides to the insurance estimate [Claim Procedures ( claimprocs )](claimprocedure.html) and are reflected in [Treatment Plan](treatmentplan.html) estimates.
...
Attachments can be added to preauthorizations as needed. If sending attachments using [DentalXChange Attachment](dentalxchangeattachmentservice.html), right-click a Preauthorization from the Treatment Plan Module, Pre Authorizations grid for attachment options. If using [EDS Attachment Service](edsattachmentservice.html) or NEA/Manual attachments, go to the [Attachments Tab](claimtabattach.html) in the Preauthorization to add attachments.
