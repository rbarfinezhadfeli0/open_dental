...
The Hidden Splits tab shows a summary of patient [Payments](payment.html) with a [Paysplit Unearned Type](definitionspaysplitunearned.html) flagged as *Do Not Show on Account*. They are typically prepayments to [Payment Plan](paymentplandynamic.html) charges or treatment planned procedures and attached to one or the other. These [Unearned / Prepayment](unearnedprepayment.html) types are hidden from the Patient Account tab, most reports, and statements, as to not affect the patient balance until the treatment is complete or a payment plan charge is due.
...
Create additional prepayment types in [Definitions: PaySplit Unearned Types](definitionspaysplitunearned.html) and assign the *Default treatment plan procedure unearned type*  in [Allocations Setup](allocationssetup.html) and *Payment plan unearned type*  in [Preferences](preferences.html).
...
When a patient overpays a [Payment Plan](paymentplandynamic.html)  charge, and opts to apply the excess to a prepayment, an income transfer is automatically created moving the overpaid amount to a hidden split. The transfer consists of a negative paysplit, subtracting the excess payment from the pay plan charge, and a positive paysplit (also attached to the Payment Plan) turning the excess into a prepayment that shows in the Hidden Splits tab.

Once the next Payment Plan charge becomes due, the prepayment transfers to the new interest charge. If the prepayment is more than the amount of the new interest charge, any excess is transferred to the principal Payment Plan charge. The transfer appears as a positive income transfer line item in the Payment Plan and a negative line item in the Hidden Splits tab.
...
If the prepayment is more than the total interest and principal charges, run the [Income Transfer Manager](incometransfermanager.html) to re-allocate it to outstanding charges outside of the Payment Plan.