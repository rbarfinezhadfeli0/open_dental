...
* **A/R Manager Excluded Grid**: For Transworld Systems (TSI) users only. Determines the columns in the TSI A/R Manager for excluded accounts.
...
* **Account Module**: Determines the columns in the [Account Module](account.html), Patient Account grid.
...
* **Appointment Edit**: Columns in the Procedures on this Appointment grid in the [Edit Appointment](aptedit.html) window.
...
* **Ortho Chart**: Columns in the lower grid of the [Ortho Chart](orthochart.html). This setup is different than other display fields.
...
* **SuperFamily Grid Columns**: Columns that display in the Family Module for a Super Family.
...
* **Up / Down**: Highlight a field, then click to move it in the order.
...
* **Allowed**: Used in the Treatment Plan Module. The PPO fee for PPO plans or the allowed fee for out-of-network plans. If a patient has multiple insurance plans, this is the fee for the primary plan.
...
* **Broken Appts**: Used in Chart Patient Information. Displays the number of broken appointments a patient has.
  + The count is based on the missed appointment procedure (D9986) if it exists in the Procedure Code List, or if it doesn't exist, adjustments with a broken appt type.
  + At least one procedure status must be checked in the Procedures area in the [Chart - Show tab](showtabchart.html).
  + When in Chart - Audit mode, also includes deleted procedures.
* **CareCredit Pre-Approval Status**: For CareCredit users. Shows the patient's pre-approval status.
* **CareCredit Pre-Approval Amount**: For CareCredit users. Shows the patient's pre-approval amount.
* **CareCredit Available Credit**: For CareCredit users. Shows the available credit.
* **Carrier Name**: Used in the [Recall List](recalllist.html). Displays the carrier name for any insurance plans currently added to the patient.
...
* **Tax Est**: Used in the Treatment Plan Module. Displays the tax estimate for [Procedure Codes](procedurecodes.html) marked *Auto Tax*.
