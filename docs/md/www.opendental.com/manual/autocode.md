...
Multiple conditions from the same category grouping should not be assigned to a single Auto Code Item, as it is not possible for multiple conditions to be met simultaneously. If the same code is used for multiple conditions in the same condition group, create separate Auto Code Items (e.g., D2394 is used for both Four\_Surf and Five\_Surf).
