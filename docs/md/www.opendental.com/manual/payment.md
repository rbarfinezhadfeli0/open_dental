...
See [Credit Card Payment](paymentpatientcreditcard.html) for instructions on processing a credit card payment using an integrated processor.
...
4. Click **OK** to proceed to the payment window (shown above) and enter payment details.
   ...
   * If a selected charge was previously overpaid, the overpaid amount is deducted from the charge and reallocated to another charge within the current payment.
5. Click **Save** to finalize the payment.
...
2. Enter the payment amount.
   * Check *Prefer this patient* to apply the payment to the patient's oldest outstanding charges. Leave unchecked to apply the payment to the oldest outstanding charges for the entire family.
3. If the *Paysplits* setting in [Allocations Setup](allocationssetup.html) is set to *Rigorous* or *Auto-Split*, Open Dental automatically creates payment splits for outstanding charges. If set to *Manual*, paysplits are made for the patient only.
4. Enter payment details.
5. Click **Save** to finalize payment.
...
7. Click **Save** to finalize payment

## Payment Details

![](images/patPaymentDetails.png)

* **Clinic**: Only displayed when Clinics are enabled in [Show Features](showfeatures.html). The default clinic is determined by *Default Clinic for patient payments* set in [Preferences](preferences.html). Click the dropdown to select a different clinic. Clinic specified determines which payment processor credentials are used for merchant services if defined per [Clinic](clinics.html). If using [Deposits](depositslip.html), this is the clinic the payment is associated with.
* **Paid By**: The name of the patient making the payment. When a payment is split to multiple family members, the payment description in the Patient Account grid shows the *Paid By* name.
...
* **Surcharge Fee**: Surcharge or merchant fee amount charged by the payment processor. Only displays for [CareCredit](carecreditpayment.html) and [PayConnect](payconnectpaymentwindow.html) 2.0 users. Does not affect patient account balance. This is a database field that can be used for reporting (i.e., [Daily Payments Report](reportdailypayments.html)).
  + For [CareCredit](carecreditpayment.html) users, the field is displayed once a transaction is processed and is read-only.
  + For [PayConnect](payconnectpaymentwindow.html) 2.0 users, the field can be edited if no transaction has been processed on the payment. This is useful for entering payments that were processed outside of Open Dental. Once a transaction is processed on the payment, the field displays surcharge fee amount attached to the transaction and is read-only.
* **Payment Date**: The date the payment is received. Used on Deposit Slips, reports, and Patient Account grid.
...
* **Note**: A note with the [Credit Card Transaction Details](paymenttransdetails.html) is automatically added after processing a credit card payment. Optionally, add a note that appears in the patient's account as the payment description in addition to the Payment Type. Notes show on statements when the *Show notes for payments* preference is enabled. [Right-Click](rightclicktextboxes.html) the text box for additional options.
* **Hide Splits/Show Splits**: Click to toggle showing the Payment Splits and Charges areas. If *Hide paysplits from payment window by default* is disabled in [Allocations Setup](allocationssetup.html), this area shows by default. See *Current Payment Splits* and *Outstanding Charges and Treatment Planned Procedures* sections below for more information.
...
* **None (Income Transfer)**: Check to mark the payment as an [Income Transfer](incometransfer.html).
  + When checked, other Payment Type options are disabled, the **Pay** button changes to **Transfer**, and the Outstanding Charges grid groups by patient, clinic, and provider.
  + This option is disabled after a credit card transaction is processed or when editing existing payments that are attached to a deposit or are not $0.
* Choose the Payment Type in the list. Customize options in [Definitions: Payment Types](definitionspaymenttypes.html). If the preference *Payments prompt for Payment Type* is disabled, a Payment Type is selected by default and can be changed. If the preference is enabled, a Payment Type must be selected manually.
...
**Apply to Recurring Charge**: Check to apply this payment to a scheduled recurring charge (i.e., currently in the [CC Recurring Charges Tool](recurringcharges.html) list). The user is prompted to select a date to apply the recurring charge. The patient is removed from the Recurring Charges List for the date the payment was applied.

## Current Payment Splits

[Payment Splits](paysplit.html) (paysplits) currently attached to this payment are listed here. Paysplits can be allocated to production (i.e., procedures or adjustments), providers, clinics, patients, pay plan charges, etc. Paysplits can also be unallocated and attached to an [Unearned / Prepayment](unearnedprepayment.html). Double-click a row to [Revise a Paysplit](paysplitsedit.html).
...
* **Clinics**: Select one or more specific Clinics attached to the payment to filter paysplits. Only Clinics with paysplits attached to the current payment are available in the dropdown.
* **Amt Min**: Enter a positive value to filter the paysplits with a value greater than or equal to this amount.
* **Amt Max**: Enter a positive value to filter the paysplits with a value less than or equal to this amount.
...
All paysplits currently attached to the payment are listed in the **Current Payment Splits** grid. The grid columns indicate the provider, patient, procedure code, and amount of the paysplit. The *Type* column indicates the type of production (procedure, adjustment, etc.) attached to the paysplit and any additional details (e.g., procedure abbreviation, adjustment type description, etc.)
...
* **Add Split**: Manually create an additional paysplit.
* **Total**: The sum of all current payment splits. The Total must match the total payment Amount to save the payment.
...
* Use the checkboxes at the top of the grid to further filter the listed charges.
...
* All paysplits matching the applied filters are listed in the Outstanding Charges grid.
  ...
  + Total: The total *Amt End* of all the selected outstanding charges. If no charges are selected, this is the total *Amt End* of all charges in the grid.
...
**Treat Plan Tab**: This tab is only available when *Allow prepayments to allocate to treatment planned procedures* is enabled in [Allocations Setup](allocationssetup.html). Lists all treatment planned procedures that meet the filter criteria.

* Amt Orig: Amount owed on a charge prior to any payments, adjustments, or insurance estimates.
* Amt End: Amount owed on a charge after all payments, adjustments, and insurance estimates including the current payment.
* Total: The total *Amt End* of all the selected treatment planned procedures. If no procedures are selected, this is the total *Amt End* of all procedures in the grid.
...
**Add Partials**: Click to create a paysplit for only part of an outstanding charge. There is a prompt to enter the partial paysplit amount after clicking this button.
...
* **Save**: Post the payment to the patient account or save changes.
* When exiting a Payment without clicking Save, the behavior varies depending on the point the user is in the payment process.
  + New payment, no credit card transaction: If this is a new payment and no credit card has been processed, exiting deletes the payment.
  + New payment with a credit card transaction: If this is a new payment and a credit card was processed, exiting voids the credit card transaction, posts the payment, and posts a separate voided payment.
  + Editing an existing payment: Exiting cancels any changes made in the payment window.