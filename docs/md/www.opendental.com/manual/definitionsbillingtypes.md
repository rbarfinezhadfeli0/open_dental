...
Billing Type options are available in the [Edit Patient Information](patientedit.html) and [Edit Insurance Plan](insplan.html) windows. Set the default Billing Type in [Practice Setup](practice.html).
...
**Hidden**: Billing Types may be hidden. However, if in use, hidden Billing Types may be visible throughout various areas of the program. If *Adding new primary insurance plan to patient sets billing type* is enabled in [Preferences](preferences.html), and an insurance plan is assigned a hidden Billing Type, adding the insurance plan to a new patient will still change the patient to the hidden Billing Type.
