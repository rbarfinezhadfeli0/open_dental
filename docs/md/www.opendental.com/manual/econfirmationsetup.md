...
eConfirmation messages can include the [AddToCalendar] tag so patients can add the appointment to the calendar on their device.

For information on adding and editing rules, see [Edit Rule](editrule.html).

## eConfirmation Exclusions

To exclude eConfirmations from being sent on certain days of the week (e.g., weekends), or certain days (e.g., holidays), set the [eConfirmation and Web Sched Recall Exclusion Days](econfirmationexclusiondays.html).