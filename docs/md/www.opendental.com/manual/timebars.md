...
Provider time, assistant time, and appointment length are calculated automatically based on the time pattern of the procedures attached to the appointment. Time and length can be changed by editing the time pattern of the appointment or the time pattern of procedures.
...
Appointment length can be set to a maximum of nine hours when using the slider in the Edit Appointment window and a maximum of 6.5 hours when adjusting the appointment box from the Appointments Module.
