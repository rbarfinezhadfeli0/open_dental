...
Click **Add** or double-click an existing Automation to open Edit Automation.
...
**Trigger**: Select the action that will trigger this Automation.
...
* CreateApptNewPat: When an appointment is created for a patient with no appointment history (new patient). Does not trigger when creating [Planned Appointments](https://opendental.com/manual/apptplanned.html).
* PatientOpen: When a patient's record is opened/selected.
...
**Conditions** (optional): The grid lists the conditions currently attached to the Automation. These must be met for the automation to trigger. See Automation Condition Edit below for details.
...
Click **Save** to keep changes or create a new automation.

## Automation Condition Edit

Click **Add** or double-click an existing condition to open Automation Condition Edit.
![](images/automationConditionEdit.png)

**Field**: The field to be checked.

**Comparison**: Determine how the condition must be met.

**Text**: Enter the description, name, text, etc. for the field being matched. If using Contains, can be partial. For certain fields, click **[...]** to select from a list.

Conditions are considered together. All conditions of an Automation must be met for the action to occur. To have automation occur for each condition, create separate Automations.

## Fields

Most fields that lare listed when adding or editing a condition are self explanatory. Below are a few that need additional explanation.
