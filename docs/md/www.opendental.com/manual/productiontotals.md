...
* To instead calculate the sum for all procedures scheduled in operatories that show in the appointment view, enable the *Appointments Module production uses operatories* in [Preferences](preferences.html).
* To calculate adjustments, including discount plan adjustments, in the net production total, enable *Add daily adjustments to net production* preference.
...
**Daily Goal**: View the total daily production goal for all providers scheduled in the appointment view. The value is calculated by multiplying the hourly production goal by the hours scheduled for the provider. Hourly goals are entered on the [Edit Provider](providerseditwindow.html) window.
...
* *Add daily adjustments to net production*: Controls whether or not adjustments, including discount plan adjustments, are included in the net production total. The net production on appointments may not equal net production total if adjustments are not attached to procedures.