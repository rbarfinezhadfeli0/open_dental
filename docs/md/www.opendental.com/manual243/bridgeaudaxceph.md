...
The AudaxCeph bridge must be open to use the bridge button in Open Dental. If a patient is selected in Open Dental, but AudaxCeph is already open with a different patient selected, clicking the bridge button does not change the patient.

## Technical Details

Because of the AudaxCeph bridge, the following occurs:

Patient Status: Every patient opened in AudaxCeph has a patient status of active, even if a different patient status (e.g., deceased, inactive) has been selected on the Edit Patient Information window in Open Dental.
...
When the bridge is enabled, Open Dental creates an update.xml file in the installation folder which AudaxCeph reads from.