...
In the [Main Menu](mainmenu.html), click Lists, Employees.

![](images/employees.png)

Employees must be created here to use the [Time Clock](timeclock.html). Employees can also be associated with [Users](securityusers.html) and [Schedules.](schedule.html)
...
For steps to take when an employee leaves the practice, see [Employee Leaving](employeeleaving.html).
...
* **Hidden**: Check this box when the employee is no longer at the office. Hides the employee from the [Time Clock](timeclock.html) and other areas that display the employees list.
...
* **Delete**: Remove the employee from the Employees list. Only employees with no [Time Card](timeclockedit.html) events can be deleted.
