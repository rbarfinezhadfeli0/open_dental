...
Only alerts the user for [Alert Categories](alertcategories.html) selected in User Edit, [Alert Subscription](alertsubscribe.html) are displayed. Additionally, if using Clinics, only alerts for the currently selected clinic are displayed.

![](images/alertShow.png) When there are new, unread alerts, the Alert item in the Main Menu appears red and indicates the number of active alerts. Alerts are updated based on the *Check alert interval in seconds* set in [Preferences](preferences.html).
...
| CareCredit Batch Errors: Errors processing CareCredit batch requests | N/A | CareCredit Batch Errors | Alert will display when there is an error when running CareCredit batch quickscreens due to a closed merchant number. | Click to open the [CareCredit Transactions](carecredittransactions.html) window. |
