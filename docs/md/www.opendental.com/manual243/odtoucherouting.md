Table of Contents

[Manual](manual.html)

[![Home](resources/logoWhite160.png)](../index.html)

Manual
v24.4 +v24.3v24.2v24.1v23.3v23.2v23.1v22.4v22.3v22.2v22.1v21.4v21.3v21.2v21.1v20.5v20.4v20.3v20.2v20.1v19.4v19.3v19.2v19.1v18.4v18.3v18.2v18.1v17.4

[![](resources/home.gif)](manual.html) [![](resources/search.gif)

Search](searchmanual.html)

ODTouch: eRouting

eRouting easily allows users to take a patient through their appointment using a customizable checklist to work through.

From the [ODTouch: Home Screen](odtouchhome.html), tap **eRouting**.

![](images/odtoucheRouting.png)

Alternatively, from the Home Screen, tap an appointment, then tap **Begin eRouting**.

eRouting is not available on mobile phone devices.

In Open Dental, users can view where a patient is in their appointment by reviewing the [eRouting](erouting.html) window.

Edit or create custom eRouting definitions in Open Dental, [eRouting Setup](eroutingsetup.html).

Appointment and patient information display at the top. Actions to take during the appointment display as a checklist. Tap an item to go to the appropriate screen, or tap the checkmark to mark it as completed.

When going through actions (e.g., charting, treatment planning, etc.), use the Patient Menu to quickly return to the eRouting checklist.

To discontinue eRouting for a patient and delete the eRouting checklist, tap the caret at the top, then tap **Delete**.