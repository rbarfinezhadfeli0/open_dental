...
Find below instructions on how to correct a [Claim](claimedit.html) with errors on the included procedure codes, or attached providers.

Correcting claims may involve editing historical data. Any changes to historical procedures, claims, or payments may affect financial history or reports. Contact [Open Dental Support](../contact.html) if you are unsure which steps are best to correct your claim.
...
* If patient payments were allocated to the incorrect procedure, an [Income Transfer](incometransfer.html) may be necessary once the correct procedure is charted to properly allocate payment.
...
1. First make a [Commlog](commlog.html) entry in the account explaining why payment denied.
...
2. Add a subtraction [Adjustment](adjustments.html) for the amount of the incorrect procedure. Associate it to the incorrect procedure.
...
4. If a check needs to be written from the practice to the insurance company, write the check, but leave the entry out of the deposit if using a [Deposit Slip](depositslip.html).
...
If you just need to change a procedure code and there is no claim attached, click **Change** on the [Procedure Info](procedureedit.html) window.
...
1. If necessary, create a [Commlog](commlog.html) entry in the affected patient account explaining why the payment was denied.
2. Open the claim and change its status to *Waiting to Send*.
3. Delete the claim.
...
2. Double-click the claim procedure from the Insurance Estimates and Payments section of the [Financial](../manual/procedurefinancial.html) tab.
3. Edit the Provider on the claim procedure, if needed.

Note: If this is not allowed, disable *Changing treating provider updates Claim Proc provider* in [Preferences](preferences.html).

## Canada Users

In some cases, a claim can be reversed if edits need made on the same day the claim was sent. In these cases, reverse the claim, make the change, then send the claim again. For more information, see [Canada Claims](canadaclaims.html).