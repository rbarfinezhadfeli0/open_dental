...
**Add**: Open Edit Carrier (see below) to create a new carrier.

**Combine**: If multiple entries for the same carrier exist, combine them. See [Combine Carriers](carrierscombine.html).

## Edit Carrier

Double-click an existing carrier to edit or click **Add** to create a new one.
...
**Electronic ID**: The electronic ID used when submitting e-claims. Insurance companies or clearinghouses provide the correct number.
...
**ERA Automation**: Determines how [ERAs](era835workflow.html) for this carrier are processed.
...
* Semi-automatic: ERAs for this carrier can be processed with a single click of the Auto Process button on the ERA window. This receives the claims associated with the ERA and finalizes the payment. ERAs can also be processed manually if needed.
...
**Consolidate Ortho Ins Payments**: Determine if ortho insurance payments are consolidated. This affects the selected insurance plan only. The global preference is determined by the *Consolidate Ortho Insurance Payments* preference.

**Carrier Group**: Only visible if carrier group names exist in [Definitions: Carrier Group Names](definitionscarriergroupnames.html). Select a carrier group to associate with the carrier. Useful for queries.

**Hidden**: Hide this carrier so it is no longer available as a selection.

**Is trusted for real-time eligibility**: Check to enable automated import of [Electronic Eligibility and Benefits](ebenefits.html) when using [Scheduled Processes](scheduledprocesses.html) for this carrier.
...
* Carrier sends patient % (default): The carrier sends the percentage covered by the patient. Open Dental imports this as the percentage covered by insurance. For example, if patient pays 20%, Open Dental imports this as Insurance Pays 80%.
* Carrier sends insurance %: The carrier sends the percentage covered by insurance. Open Dental imports these benefits as is.

**Ins Plan Subscribers**: Indicates how many insurance plan subscribers are associated with this carrier. Click the dropdown to see all current and previous subscribers.

**Delete:** Only carriers not attached to any insurance plans or subscribers can be deleted. Usually, this is a carrier that was added but never used. For duplicate carriers attached to subscribers, use [Combine Carriers](carrierscombine.html) from the Carriers list instead.