...
The [Fee Schedule](feeschedules.html) used for procedure fee estimates is determined using the following logic.
...
Priority 3: Fee Schedule of the provider assigned to the procedure's appointment or any appointment scheduled today.
...
If the insurance plan is mixed capitation ([HMO / DMO Insurance Plan with Co-pays](planhmosupplemental.html)), the same logic applies as for PPO Plan Types as described in the preceding section since these plans use the *PPO Percentage* Plan Type.
...
- Procedures marked *Do Not Bill to Ins* still use the fee logic for the insurance plan assigned to the patient.
...
Setting up overrides is helpful in the case of specialist or clinic specific fees and should be done instead of setting up individual Fee Schedules for clinics or providers.
