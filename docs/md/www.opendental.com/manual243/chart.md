...
* **eRx:** Create an electronic prescription. If using DoseSpot, opens the [DoseSpot eRx / Prescription](dosespotuse.html) interface. If the logged-on user is a Proxy user, a prompt displays to select a provider before launching the DoseSpot interface. If using Ensora eRx (formerly NewCrop eRx), opens the [Ensora eRX / Prescription](newcrop.html) interface. If eRx is not enabled for the practice, an error displays. Click the dropdown to view additional options.
