...
Before a prescriber can create or send an electronic prescription via the [Ensora eRx (formerly NewCrop eRx) / Prescription](newcrop.html) interface, necessary information must be entered into Open Dental.

General requirements to use Ensora eRx:
...
If any of the following requirements are not met when enabling Ensora eRx, users get an error message when clicking the **eRx** button in the Chart Module.

* To enable or create prescriptions, the logged-on user must have the *Rx Create* permission. See [Permissions](permissions.html).
* Due to strict data requirements, the following information must be entered before a prescription can be sent:
