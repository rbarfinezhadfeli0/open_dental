...
The office can determine which type of appointments a new patient can schedule, what hours are available, what operatories are available, and other appointment criteria. Provider schedules must be created prior to utilizing Web Sched New Patient (see [Schedule](schedule.html)).
...
* **Prevent double booking**: Check to block double booking of all Web Sched New Patient appointments. Uncheck to use [Appointment Rules](appointmentrules.html) to block double booking of specific procedures.
