...
**Show Fee Schedule Groups**: For Enterprise users only. Only visible if *Show Fee Schedule Groups* is enabled in [Preferences](preferences.html). Changes the Clinic dropdown to Fee Schedule Group. Check to allow edits to fee schedules grouped by clinic.
...
**Fee Tools**: Copy, export or import fees, increase fees by a percentage, or update fees for treatment planned procedures. See [Fee Tools](feescheduletools.html).
...
* Fee changes are not immediately reflected on charted (e.g., treatment planned) procedures. See [Fees Update](feesupdate.html) for information on updating fees for treatment planned procedures.
* An [Audit Trail](audittrail.html) entry is made when a fee is changed.