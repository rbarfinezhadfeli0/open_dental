...
BioPAK uses the patient number to identify selected patients. Open Dental sends the selected patient's birthdate, chart number (if selected), gender, patient number, and name to BioPAK. If the patient number does not exist, a new patient record is created. If the patient number does exists but the name does not match then it gives an error that the patient already exists. If the patient record is correct, update the name to match in both softwares.