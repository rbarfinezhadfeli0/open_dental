...
![](images/securityGlobalSecuritySettings.png)

**Time Card Security Enabled:**

* Checked: Enables the *Users cannot edit their own time card* and *Users cannot edit their own time card except current pay period* boxes (see below). Limits users without the *Edit All Time Cards* permission from viewing other employee timecards or creating clock events for other employees.
* Unchecked: Disables the *Users cannot edit their own time card* and *Users cannot edit their own time card except current pay period* boxes. Allows users to create clock events (e.g., Clock In) for any employee. Allows all users to view any employee Time Card.

**Users cannot edit their own time card:** Can only be changed if the Time Card Security Enabled box is also checked. The setting of this box overrides the *Edit All Time Cards* security permission.
...
**Users cannot edit their own time card except current pay period**: Can only be changed if the *Time Card Security Enabled* box is also checked.
...
* Unchecked: Users are not required or prompted to change to a strong password.

**Manually enter log on credentials:**

* Checked: Requires users to manually type their user name when logging in.
* Unchecked: Users select their user name from a list.

**When logging in with a badge, require a password**:

* Checked: If using [Badges](securitybadges.html) to [Log On](securitylogin.html) to Open Dental, a user is selected when scanning a badge. Users must type their password.
* Unchecked: If using badges to log on, the user is immediately logged on to Open Dental when scanning a badge.

**Maintain selected patient when changing users**: Only visible when [Clinics](clinics.html) is enabled.

* Checked: When a user [Logs Off](securitylogin.html), the previously selected patient, as well as the 5 previous patients in the dropdown menu, remain selected for the next user who logs on.
  + If the previously selected patient is not assigned to a clinic, the patient is not maintained.
  + If the next user to log on does not have access to the clinics associated with the previously selected patients, those patients do not show in the list.
