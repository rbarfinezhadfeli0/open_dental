...
* **Export Grid**: Click to export the Employee Time Cards grid as a .txt file. Users are prompted to select a folder for export. When the export is complete, a message indicates the file name and location.
* **Export ADP**: Click to export the time card data in ADP Workforce Now format (.csv file). Users are be prompted to select a folder for export. If any errors are detected, a notification is shown. When the export is complete, a message indicates the file name and location. To include employee names when exporting, enable *ADP export includes employee name* in [Preferences](preferences.html).
* **Export ADP Run**: Click to export the time card data in ADP Run format (.csv file). Users are prompted to select a folder for export. If any errors are detected, a notification is shown. When the export is complete, a message indicates the file name and location. Requires *ADPRunIID* to be entered in [Time Card Setup](timecardsetup.html).
...
* **Daily**: Calculate total hours worked each day for all employees, taking into account rules for overtime, differential hours, and setting for breaks over 30 minutes. Adjustments are made within a clock event; new rows are not created. The current day is excluded if it falls within the pay period being calculated.
...
2. Calculate total hours worked each day, taking into account rules for overtime, differential hours, and the setting for breaks over 30 minutes. Adjustments are made within a clock event; new rows are not created.
