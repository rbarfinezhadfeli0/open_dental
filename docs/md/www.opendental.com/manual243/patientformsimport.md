...
**Address fields**: If the preference *Verify Addresses with USPS* is enabled, the address is verified when importing.

* If a recommended address is found, the Address Compare opens. Click Yes to import the recommended address. If No is clicked, an address is not imported
* If no recommended address is found, a warning is show indicating *Failed to verify address with USPS*. Click Yes to continue importing the address. Click No to return the import window. To proceed with importing information other than the address, uncheck *Do Import* for any address lines.