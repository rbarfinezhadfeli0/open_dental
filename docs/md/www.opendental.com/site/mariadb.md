...
* If using Windows 8.1 or Windows Server 2012 the following Windows update must be installed: [KB2999226](https://support.microsoft.com/en-us/topic/update-for-universal-c-runtime-in-windows-c0514201-7fe6-95a3-b0a5-287930f3560c)
* MySQL 5.5, MySQL 5.6, or MariaDB 10.5 must be installed and currently functional.
* Adequate storage space to create two full backups of the \mysql\data\ folder, plus an additional 220MBs.
* Open Dental version 24.3 or greater is required for MariaDB 10.11.

**Upgrade Steps**:

1. Clean up the **mysql/data** folder. See [MySQL Data Directory Management](mysqlmanage.html) for details.
2. Ensure Open Dental is closed on all workstations. On the server, log into Open Dental and go to Tools, Misc Tools, [Shutdown All Workstations](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7shutdown.html), **Shutdown All**.
...
When launching Open Dental from a workstation, error "Object cannot be cast from DBNull to other types" may occur if the upgrade wizard was run prior to updating each instance of Open Dental. To resolve, run the setup.exe file found in the [A to Z Folder](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7atozfolder.html).
