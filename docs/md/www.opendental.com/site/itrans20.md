...
9. Enter the Service User Account and password. This is the user that CDA Digital IDs (i.e., provider certificates) will be installed to. Then click **Next**.
   ...
   Note:
   * Domain, user, and password must be entered correctly for the service to install.
   * If not using a domain, enter the server name instead (e.g., server\user or .\user).
   * If the user associated with the ITRANS service is a domain user, it is recommended to set the ITRANS Claims Director service's startup type to *Automatic (Delayed)* to prevent the service from failing to start.
