...
2. **Install the *OpenDentalReplicationService* to each Replica**. See [Open Dental Replication Service](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7opendentalreplicationservice.html).
...
12. Launch Open Dental on Primary Server using LocalHost and the replicated database. Input report server settings. See [Report Setup: Report Server](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7reportserver.html).
13. Ensure replication is working as intended by performing the following:
    * On the Primary Server, create an appointment for a test patient and make note of the patient's Patient Number, see [Edit Patient Information](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7patientedit.html). Verify the appointment was created on the Report Server by querying it from the Primary Server. Go to Reports, User Query, and ensure that the *Run on report server* box is checked before running the following query: SELECT \* FROM appointment WHERE PatNum = [Patient's PatNum]
