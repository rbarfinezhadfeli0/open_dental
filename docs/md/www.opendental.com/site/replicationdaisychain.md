...
* It is recommended to use a folder synchronization software for the [A to Z Folder](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7atozfolder.html) when using replication. See [Online Backups](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7backupsonline.html).
...
1. **Add servers and install the *OpenDentalReplicationService* to each Replica**. See [Replication Setup](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7replicationservers.html).
    You must have the [Open Dental Replication Service](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7opendentalreplicationservice.html) when running Daisy Chain Replication. The service will notify all users and IT staff if replication crashes via [Alerts](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7alerts.html). We are not responsible for the damage done when databases continue to be used after an error crashes replication.
...
8. **Set each replication server's Primary (Master)**. Go to the command line and navigate to the appropriate bin folder of the MySQL service (typically **C:\Program Files\MySQL\MySQL Server 5.5\bin** or **C:\Program Files\MariaDB 10.5\bin**) and type the following: mysql -u root opendental where opendental is the name of the database. If you have a MySQL user password, add -p to the command.
...
10. **Run START SLAVE; in the mysql command interface for each replication server in the chain**.
11. **Run SHOW SLAVE STATUS\G in the mysql command interface for each replication server in the chain**. Make sure that the Slave IO Running and Slave SQL Running columns both say, Yes.
