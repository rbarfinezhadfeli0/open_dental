...
**Open Dental Backup Tool (Hot Method)**: Use the built-in [Backup Tool](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7backuptool.html) to routinely make backups of the entire database. The tool can be used for backups only, not restores.
...
**Network Path Supplemental Backups (Hot Method)**: The [Supplemental Backups](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7supplementalbackups.html) utility in Open Dental can be used to create an automated, encrypted, zipped, database dump in a network or local location nightly. Network Path Supplemental Backups are a complete copy of a database but are not recommended to be relied upon as the primary backup method. The OpenDentalEConnector service performs this backup once every 7 days.
...
**Alternative InnoDB Backup and Restore Methods Using Third-Party Software (Varies)**: A backup and restore can also be achieved using 3rd-party software such as MySQL Workbench or SQLYog. Methods may be Hot or Cold and can vary by software. Please reference their documentation for more information. Also see: [Online Backups](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7backupsonline.html).