...
* Visit [Computer Requirements](computerrequirements.html) for a full list of supported operating systems.
...
See [Screen Size](screensize.html) for additional information on screen scaling, resolution and display size.
...
2. Make sure you are using the most current version. In the Main Menu, click Help, Update. Check for Updates to check. If a new build is available for your version, update.
