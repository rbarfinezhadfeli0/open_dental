...
* Check the database connection status and version in contrast to CEMT database version (upper left).
...
* **Setup**:
  ...
  + **Report Permissions**: Control user access to individual standard reports, by user group. See [Report Setup: Security Permissions](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7reportsecurity.html).
  + **Security**: Define security settings (users, user groups, permissions, and lock dates). See [CEMT Security](cemtsecurity.html).
  + **Display Fields**: Customize which columns of information show when searching for patients. See [Display Fields](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7displayfields.html).
...
* **Help**
  + **Query Monitor**: Launch [Query Monitor](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7querymonitor.html). Once started, monitors action from within CEMT only.
  + **Payload Monitor**: Only for practices utilizing [Middle Tier](middletier.html). Launch [Payload Monitor](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7payloadmonitor.html). Once started, monitors payloads from within CEMT only.
...
Solution: Ensure that the user currently signed into CEMT has a password set.

Solution: Ensure that the username and password exist in the database connection. If unable to push the credentials for the user from the Central Manager, the CEMT connection can instead be changed to a direct connection to try to push the users through.
...
Solution: Override high DPI scaling behavior in Windows for CentralManager.exe (not for OpenDental.exe). See [Zoom](../autoLogin.aspx%EF%B9%96ReturnUrl%3D%EA%A4%B7help253%EA%A4%B7zoom.html), Turn Off DPI Awareness, Option 2 for instructions.