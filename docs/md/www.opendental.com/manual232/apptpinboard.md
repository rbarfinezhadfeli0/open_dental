...
Multiple appointments can be on the Pinboard at one time. Appointments display on the Pinboard for the instance of Open Dental they were added only. If a user logs off and a new user logs on, appointments remain on the Pinboard. Appointments are cleared from the Pinboard when Open Dental is closed
...
**Openings in View**: The first available time slots is listed for each of the next ten available dates. Click a date to quickly display the selected date on the schedule. See Search Behavior at the bottom of this page for additional information on which openings are considered.
...
**Search Results**: Lists up to 15 available appointment dates meeting the search filters. There may be additional openings on a specific date; only the first available time slot is listed for each date. Click a date to quickly display the selected date on the schedule.

Filters: Set criteria to search for available appointments dates.
...
Click **Close** to exit the Advanced Appointment Search. Click and drag an appointment from the Pinboard to schedule it.

## Search Behavior

The *Search Behavior* [Preference](preferences.html) will affect what openings appear in the list.
...
* If set to ProviderTimeOperatory: The search will considers the provider's scheduled availability, available provider time, already scheduled appointments and *Block appointments scheduling* blockouts in the provider's scheduled operatory.

The way a schedule is setup may affect Search behavior. See [Schedule Setup Examples](schedulesetupexamples.html) for details.
