...
1. In the [Main Menu](mainmenu.html), click the New Claim dropdown, **Other**. ![](images/claimCreateOther.png)
   ...
   All plans the patient is currently subscribed to list. To list all insurance plans for the family, including inactive or dropped plans, check **Show plans for family which are not in use by the current patient**.
...
If a patient has an insurance set up with of:

1. Dental (Primary)
2. Medical
3. Dental (Secondary)

Follow the steps below to create a claim for all insurance plans.

![](images/claimTypeExample.png)

1. Click **New Claim** to create the Primary Dental insurance and the Secondary Dental insurance.
