...
* **User Query**: When checked, the window displays a Query box instead of filters. Enter the query, then click **Refresh** to see results.
  ...
  + SET statements cannot be used and will cause an error.
...
The list of patients matching the filter criteria will show. Select which patients will receive the communication.

* **Set All**: Mark all patients to send to.
* **Set Selected**: Set only the highlighted patients to send to.
