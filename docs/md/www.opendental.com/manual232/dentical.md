...
Following review, EDI Support will let advise if there is anything wrong with the batch, or if additional test claims are needed. EDI Support will notify users when the batch meets their test requirements and when cleared for live claims in the production environment.
