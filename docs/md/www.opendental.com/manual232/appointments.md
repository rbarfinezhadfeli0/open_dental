...
* **Make Appt**: Schedule an appointment.
  ...
  + If the patient has scheduled, broken, unscheduled, or planned appointments, Appointments for Patient window opens so appointment history can be reviewed before scheduling.
* **Make Recall**: Schedule the patient's next [Recall Appointment](recall.html). Patient must be due for a *special* [Recall Types](recalltypes.html).
  + To schedule recall for a family, use the dropdown and select **Make Fam Recall**.
  + If the patient has a scheduled appointment or an appointment on the [Unscheduled List](unscheduled.html), the Appointments for Patient window opens. Click **Schedule Recall** or **Entire Family** to create appointment(s) on the [Pinboard](apptpinboard.html).
  ...
  + Recall procedures that are not attached to a *Special* Recall Type are added to appointments automatically along with the *Special* Recall if they are due on or before the same date and *Append to Special* is checked for that Recall Type.
  + If the schedule is *Day View*, the Search function is brought up.
    ...
    - When using **Make Fam Recall**, Open Dental automatically searches for scheduled openings after the last Recall Due Date for all family members and searches for openings with all family members' assigned providers. The calendar moves to the first available date. Additional dates are listed in *Openings in View*.
* **Unsched**: Send the selected appointment to the Unscheduled List.
* **Break**: Select an appointment and click to Break Appointment [Break Appointment](apptbreak.html).
* **Complete**: Select an appointment and click to Set Appointment Complete [Set Appointment Complete](apptcomplete.html).
* **Delete**: Select an appointment and click to Delete Appointment [Delete Appointment](apptdelete.html).
...
* **Month and Year**: Click and enter a specific date when prompted to move the schedule to.
* Click a day on the calendar to update the schedule to show that day's appointments.
* **M**: Click move the schedule back or forward one month from the currently selected date.
* **W**: Click to move the schedule back or forward on week from the currently selected date.
...
* **Week**: Grid is divided into seven days (Monday through Sunday). Appointments appear narrow, so hovering over the appointment to view bubbles may be useful. Move appointments within the current week by dragging and dropping, or use the Pinboard. Set the *Week View start day* in Preferences.
...
**Time**: Use the vertical scrollbar to move through the entire 24-hour day. Time increments set globally in [Appointment View Setup](appointmentviewsetup.html).
...
* Current time is indicated by a horizontal time line that automatically moves down every minute. Customize  *Appointment time line color* in Preferences.
* Vertical [Time Bars](timebars.html) show as color-coded visuals of appointment length and provider time.
...
**Colors:** By default, white areas indicate open times, grey indicates closed times. Times are determined by [Schedule Setup](schedule.html). These colors can be changed in [Definitions: Appointment Colors](definitionsappointmentcolors.html).

**Appointments:** Basic appointment details show in an appointment box. Double-click the box to open the [Edit Appointment Window](aptedit.html) or right-click to select more options. Every appointment has a second optional provider for hygiene so that only one appointment has to be made even if a patient is technically seeing two providers.

* Depending on Preferences, the appointment bubble may appear when hovering over an appointment. To customize the appointment bubble, see [Display Fields](displayfields.html).
...
* When moving an appointment to a different Operatory, users are prompted to update the provider if the current appointment provider differs from the Operatory.
...
* *On an Appointment*:
  ...
  + Go To Ortho Chart: Open the [Ortho Chart](orthochart.html) for the selected patient. Only shows when information exists in the Ortho Chart or this option is enabled in [Ortho Setup](orthosetup.html). This menu item may change. The text is based on the name of the first tab set up in [Ortho Chart Setup](orthochartsetup.html) (e.g., if the tab name is *Orthodontic Patient Chart*, the option will be *Go To Orthodontic Patient Chart*).
  ...
  + Send eClipboard BYOD Text: Send a text message to the patient that includes a link to eClipboard BYOD, if allowed in [eClipboard Setup](eclipboardsetup.html). Allows patients to download eClipboard to their own device in order to check in.
...
* *On an Empty part of the Schedule*:
  + Blockout Options: For detailed breakdown of these options see [Blockouts](blockouts.html).
