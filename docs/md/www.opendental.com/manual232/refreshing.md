...
You have control over how frequently each workstation checks for new signals. In the [Main Menu](mainmenu.html), click Setup, [Miscellaneous](miscsetup.html). For Process Signal Interval, set the number of seconds between checks to the database. If you have a very large clinic (over 50 computers) you might need to increase the time a little bit. Otherwise, anywhere between 6 and 20 seconds should work well. Leave it blank to disable the automatic refresh. If you set a value for process signal interval, you can also set a value, in minutes, that will disable the automatic refresh when the workstation is inactive.
