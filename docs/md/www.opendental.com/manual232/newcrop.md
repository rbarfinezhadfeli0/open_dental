...
* Click **Help** in any NewCrop eRx tab to open the help guide for the current tab.
* Go to the **Admin** tab to find additional Support Documentation for NewCrop eRX.
* To electronically prescribe controlled substances the prescriber must go through additional identity proofing and set up two-factor verification. See [NewCrop eRx EPCS Setup](newcropepcs.html).
...
If using NewCrop eRx Comprehensive, the monograph, leaflet, and patient education links show at the right. Click to access.4. Enter the Sig information. Some information may automatically populate. Edit as needed. Frequency must always be selected. If prescribing a Drug Set or Compound, this window does not appear.
...
![](images/newCropPending.png)5. Click **Prescribe**.
   * When using New Crop eRx Comprehensive, the system checks for interactions with current medications, allergies, and diagnoses and displays applicable alerts. See [NewCrop eRx Drug-Drug, Drug-Allergy Interaction Checks](newcropdruginteraction.html). If necessary, edit medications.
...
8. The prescribing provider must enter their 4-digit pin. If this is the first time the prescriber has transmitted a controlled substance, they are be prompted to create a pin instead.
...
* To verify if a prescription has gone through or see if it has failed, see [Report a Failed or Missing Prescription in NewCrop eRx](newcropmissingrx.html). Offices should periodically check for rejected prescriptions, so no prescriptions slip through the cracks.
* When a medication is marked as discontinued in NewCrop eRx, the medication is also marked discontinued (a *Stop Date* entered) in Open Dental when the Chart Module is refreshed.
