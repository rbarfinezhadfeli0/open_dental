...
**Enter a few letters of the insurance carrier:** Enter a carrier name to filter the report for a specific carrier. Users must enter at least one letter to proceed with running the report; a full carrier name is not required. Only carriers with names including the matching text will be included; the full carrier name is considered (e.g., if "Med" is entered both MedDent Insurance and NC Medicaid will be included on the report).

**From Date / To Date:** Enter the report date range. The current month is the default. Applies to procedure completed date.

## Report Preview

Click **OK** to generate a print preview the report. Procedures marked *do not bill insurance* are not included on the report. Below is an example of the report and explanation of the report columns. For a description of toolbar buttons, see [Complex Report System](reportcomplex.html).

![](images/reportCapitationPrint.gif)
...
* **Date**: Procedure date.
* **UCR Fee**: Usual and customary rate (e.g., office fee) fee. UCR fees are prioritized in the following order, including any [Fee Override for Provider or Clinic](procedurefeeoverrides.html):
...
* **Co-Pay**: If a co-pay fee schedule is assigned to the insurance plan, this is the amount from the co-pay fee schedule (e.g., patient responsibility).