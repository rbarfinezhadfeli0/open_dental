...
Note: In 2012, most carriers switched to each patient being their own subscriber with a subscriber ID. If the practice plans to send e-claims in 5010 format, patients may need to be dropped from their family's plan and recreate them with subscriber as self, using the patient ID instead of the subscriber ID.
...
This information is specific to the insurance plan and can only be edited by users with the *Insurance Plan Edit* [Permission](permissions.html). Carrier information can only be edited by users with the *Carrier Edit*  permission.
...
**Electronic ID/Payer ID**: Provided by the insurance company if they accept [E-Claims](clearinghouses.html). Enter the ID manually or click Search ID to search the Payor ID list. If the carrier does not accept electronic claims, there are two choices.
...
* **Exclusions:** Only for PPO plan types. If a procedure is excluded from insurance plan, determines if this insurance plan automatically zeroes out write-offs. Exclusions are defined using [Other Benefits](otherbenefits.html), or by setting coverage at 0%.
...
The Ortho tab shows when *Show Auto Ortho in account module* is selected in [Ortho Setup](orthosetup.html). Use it to enter plan information for orthodontic claims. This information is also in the Auto Ortho tab. Information can only be changed by users with the *Insurance Plan Ortho Edit* security permission.
...
**Effective Dates:** The effective dates of the insurance plan. The effective start date is required when using waiting periods. The end date does not terminate the plan; A user must drop the plan if it is no longer in use. If using [Automation](automation.html), the effective end date is used for the Condition, *Insurance Not Effective*. Set benefit renewal dates (calendar year or service year) in the Benefit Information section (see below).
