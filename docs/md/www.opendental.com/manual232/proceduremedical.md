...
![](images/procedureInfoMedical.gif)

The Medical tab appears if [Medical Insurance](medicalins.html) is turned on in [Show Features](showfeatures.html).

**Medical Code:** Corresponding medical code for the dental procedure. If a default medical code is set on the [Procedure Code](procedurecodeedit.html), it will automatically show. Required when sending a medical e-claim.
...
**Is Emergency**: Check this box when the procedure is a medical procedure. When the *P1IsEmergency*field is added to the [HCFA 1500 Claim Form](claimform1500.html), the procedure will be marked as emergency.

**SNOMED CT Body Site:** Only visible if [EHR](../site/ehrlicense.html) is turned on.
...
**Ordering Provider Override:** Ordering provider for procedure on claim. Provider will override any other providers. By default, there is no override and the treating provider is used. See [E-Claims Complexities](eclaimscomplexities.html), Ordering Provider, for the logic.
