...
For EHR, every problem in the master list must be assigned a SNOMED CT code. This will ensure that medication reconciliation and clinical decision support interventions work correctly, and that problems show on a patient's clinical documents (e.g. [EHR Continuity of Care Document ( CCD )](ehrccd.html)).
