...
Use the filter options to change the ERAs listed in the ERAs grid. Click **Refresh** to update results after making changes. Filter options may change based on [Enterprise Setup](enterprisesetup.html) Manage settings.
...
* **Control ID**: Filter by Control ID. Only shows if enabled in [Show Features](showfeatures.html).
