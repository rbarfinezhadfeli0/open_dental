...
Alternatively, this window opens when clicking **OK** to finalize a payment from the [Edit Insurance Payment](editinspayment.html) window.
...
* **Up/Down Arrows**: Highlight a claim and click to move in the grid.
...
Click **OK** to finish finalizing an insurance payment or save changes.

## Finalizing Insurance Payments from Manage Module

When finalizing a Batch Insurance Payment using the **Batch Ins** button from the Manage Module (see [Batch Insurance Payment](claimpaymentbatch.html) for detailed instructions), the window in the screenshot below is displayed. Fields described above are the same, additional fields are described below.

![](images/claimbatchattach.png)

**Attach**: Select a claim from the All Oustanding Claims Grid and click to attach to the [Batch Insurance Payment](claimpaymentbatch.html).
...
* Payment: Total unfinalized payment amount of the claim. Claims that haven't been received have a payment of 0. If there is a payment amount, the claim has been received, but not attached to a payment.

**Close**: Click to exit the window. Changes are saved.