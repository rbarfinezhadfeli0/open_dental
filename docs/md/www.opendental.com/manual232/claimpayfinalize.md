...
7. Click **OK** on the [Insurance Payment (EOB)](inspayeob.html) window to finish finalizing payment.
...
9. Click **OK** to finish finalizing payment.

Alternatively, batch insurance payments can be finalized from the [Manage Module](manage.html) using the [Batch Insurance Payment](claimpaymentbatch.html) feature. If  *Finalize claim payments in Batch Insurance window only* is enabled in [Preferences](preferences.html), users must finalize insurance payments from the Manage Module using the Batch Insurance Payment feature.
