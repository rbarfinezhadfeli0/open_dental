...
See [Sheet Checkbox and Radio Button](sheetscheckbox.html).

Checkboxes are [Sheet Field Types](sheetsaddelements.html) used to indicate a selection by toggling an *X* on or off. The available checkbox fields vary by sheet type.
...
| checkMed1 through checkMed20 | Used as a "No" checkbox. When importing, corresponding inputMeds are not added to the patient's Medications in the Medical tab. If the medication was previously active, today's date is added as the *Date Stop* in the [Medication for Patient](medicationspatient.html). |
