...
If there are insurance plans that use the PPO percentage plan type, and the office tracks write-offs, there are three options that affect when write-offs in reports are applied to production. See [Show Insurance Writeoffs](writeoffstoproduction.html) for guidance and examples.
...
**If the office has PPO insurance plans**: Determine how the practice would like to calculate write-offs. There are three options. See [Show Insurance Writeoffs](writeoffstoproduction.html) for information on each option and how it functions. .
...
* Patient prepays (See: [Unearned / Prepayment](unearnedprepayment.html)).
  ...
  + If using accrual accounting, see [Accrual Basis Accounting](../manual/accrual.html).
