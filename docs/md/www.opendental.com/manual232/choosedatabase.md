...
Alternatively, this window may display when launching Open Dental. This window always displays the first time when launching Open Dental after install.

These settings allow the workstation to locate and connect to the [MySQL](mysql.html) database.

Note: Open Dental Cloud users cannot change their database or view MySQL User information. All settings in this window will be read only.
...
Instead of connecting directly to the database, practices have the option to connect to a Middle Tier which handles all of the database interactions. The advantages include better security and speed for multiple offices. Using the middle tier makes it impossible for the user to connect directly to the database. The user never has access to the MySQL username and password used to access the database. It also prevents injection attacks when writing queries.
...
* Multiple locations: Practices with multiple locations can remote into each other without having to make sure every site is on the same exact version.
...
* Verifying backups: Users can launch a backup of Open Dental on a workstation using Dynamic Mode to verify a backup without it needing to process a database update.
