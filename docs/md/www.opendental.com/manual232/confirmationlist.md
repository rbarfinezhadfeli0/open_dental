...
* Define postcard layout in [Setup Recall](recallsetup.html).
...
* When an email is sent, the confirmation status is changes to the status selected for *Status for emailed confirmation* in Confirmation Setup.
...
* The practice must be signed up for [Texting](textintegratedusage.html).
* Patients must have a wireless phone number and be set to accept text messages (Edit Patient Information window, Text OK).
* Customize the message of the text in Confirmation Setup.
...
![](images/confirmationReport.gif)

Highlight the fields to include in the report, then click **OK**. The [User Query](queryoverview.html) is launched where data can be exported.