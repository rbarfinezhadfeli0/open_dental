...
Enter the new note in the **Appended Note** text area, or add an [Auto Note](autonotes.html). Appended notes are dated, and appear in the Progress Notes below the procedure. Appended notes can be signed electronically, see [Electronic Signatures](electronicsignatures.html). The user and signed status shown in progress notes will be that of the user to last append the procedure.
