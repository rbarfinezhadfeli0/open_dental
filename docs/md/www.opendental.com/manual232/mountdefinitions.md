...
Mount Definitions (Mount Defs) that are created here are templates. They will be available in the top toolbar of the [Imaging Module](images.html). Once a user picks a Mount Def from that toolbar, a new Mount will be created for that patient with a layout based on the Mount Def template.
...
If adding a new Mount Def, the user will start by generating a layout from one of the built-in templates. These will serve most purposes or can be a starting point for a similar Mount Def.
...
* **Measurement Scale**: The scale when measuring lines on this mount. See Calibrating Scale on [Draw](draw.html) for details.
  ...
  + **Scale, pixels per unit**: Enter the scale for this mount. All images using this mount will have the same scale as all images are taken with the same machine.
  + **Decimal Places**: The decimal places to count to when measuring lines on this mount.
* **Generate**: Generate a new mount to replace the one currently selected.
...
* **X/Y**: Enter the X and Y positions on the template. X = horizontal, Y = vertical.
