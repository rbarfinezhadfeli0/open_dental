Table of Contents

[Manual](manual.html)

[![Home](resources/logoWhite160.png)](../index.html)

Manual
v24.3 betav24.2v24.1v23.3v23.2v23.1v22.4v22.3v22.2v22.1v21.4v21.3v21.2v21.1v20.5v20.4v20.3v20.2v20.1v19.4v19.3v19.2v19.1v18.4v18.3v18.2v18.1v17.4

[![](resources/home.gif)](manual.html) [![](resources/search.gif)

Search](searchmanual.html)

EHR Vaccine Drug Units

Drug units must be set up before you can enter a patient vaccine.

In the [Main Menu](mainmenu.html), click Setup, Chart, [EHR](ehrsetupwindow.html), then click **Drug Unit**.

![](images/ehrDrugUnit.gif)

Click Add, or double click an existing unit to edit.

![](images/ehrDrugUnitEdit.gif)

Define the drug unit.

* **Unit Identifier**: Enter a unit that matches a valid UCUM code. Measurements will be validated against UCUM codes before vaccine data is exported.
* **Unit Text**: Enter text to describe the drug unit.

Click OK to save.