...
Use this menu to select the clinic. Only clinics the logged-on user has access to are listed as options. To set the default Main Menu clinic when logging on, see [Miscellaneous Setup](miscsetup.html), *Track Last Clinic by*.
...
**Patients with No Assigned Clinic**:
 To find patients with no assigned clinic, run the [Database Maintenance](databasemaintenance.html) tool and check results for PatientsNoClinicSet. If patient's are found, then manually assign a default clinics. If needed, contact Open Dental Conversions for a service to associate clinics
