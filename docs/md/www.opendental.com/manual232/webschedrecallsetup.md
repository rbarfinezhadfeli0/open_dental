...
Web Sched Recall allows a patient to schedule their recall appointment when they receive a recall reminder via email or text. To begin using Web Sched Recall, first [Sign Up](eservicessignup.html) for the service.

Note: Offices will still need to check the recall list periodically, as some patients may not meet the criteria for automated reminders. Users can verify reminders are going out by looking at the Reminders Tab in the Recall List where sent automated reminders are shown.
...
**Edit**: Click to open [Recall Types](recalltypes.html). Edit Recall Types as needed.
