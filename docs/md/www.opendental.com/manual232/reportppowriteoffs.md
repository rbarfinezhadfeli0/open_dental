...
This report will list standard fees, PPO fees, and write-off amounts.
...
**Calendar:** In the left calendar select the start date. In the right calendar select the end date. The date range defaults to last calendar month.

Group Results: Select an option to determine how results are grouped.

* **Individual Claims**: List fees and write-offs by claim. Shows patient name.
...
* Using initial claim date for write-off estimates, insurance pay date for write-off adjustments: Apply write-off estimates on the day of the insurance claim and changes to write-offs estimates once an insurance payment is entered. Shows write-off information on the claim date only. Includes additional columns on the report. See below for details.

## Report

After setting report options and filters, click **OK** to generate a print preview of the report. The report will differ depending on the grouping behavior. Below are examples of the report and explanation of the report columns.
...
![](images/reportPPOWriteoffClaim.gif)

Procedures billed to multiple PPO insurance plans, will be shown on separate rows. The *StandFee* for the procedures will be included in all the rows for additional claim(s).

The following columns only appear when *Individual Claims* is selected:

* **Date**: Write-off date. This will differ depending on the selected *Show Insurance Write-offs* behavior.
* **Patient**: Last name and first name of patient associated to claim.
...
![](images/reportPPOWriteoffCarrier.gif)
...
* **Stand Fee**: Standard fee. (e.g., office fee, UCR fee, etc.). This total may not match [Production and Income](productionincome.html) reports as procedures billed to multiple PPO insurance plan types will be accounted for in this total for each PPO claim sent.
...
* **PPO Fee**: Fee from *Fee Schedule* assigned to [Insurance Plan](insplan.html) (e.g., insurance fees, contracted fees).