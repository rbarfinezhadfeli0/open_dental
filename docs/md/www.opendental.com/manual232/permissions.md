...
| Auto/Quick Note Edit | AutoNoteQuickNoteEdit | Create or edit [Auto Notes](autonotes.html) and [Quick Paste Notes](quickpastenotessetup.html). |  |  |  |
| **Definitions** |  |  |  |  |  |
...
| Commlog Edit | CommlogEdit | Edit [Commlog](commlog.html) entries. | X | X | CommlogDateTime |
...
| Appointment Move | AppointmentMove | Drag scheduled or completed appointments to new operatories. Move appointments to the [Unscheduled List](unscheduled.html). |  |  |  |
...
| Pay Split Create after Global Lock Date | SplitCreatePastLockDate | Add new [Pay Splits](paysplit.html) for payments created before the Global Lock Date. Does not allow editing or deleting of existing payments. The sum of all splits must equal the payment amount. |  | X |  |
| Payment Plan |  |  |  |  |  |
| Pay Plan Edit | PayPlanEdit | Create, edit, close, or delete [Payment Plan](paymentplanpatient.html). Logs an entry in the Audit Trail when a plan is created, reopened, signed, or closed, when a signature is invalidated, or when the guarantor, amount, insurance plan, or note is changed. |  | X | PayPlanDate |
| Pay Plan Charge Date Edit | PayPlanChargeDateEdit | Edit the date of a payment plan charge on payment plans with APR. |  |  |  |
| Adjustment |  |  |  |  |  |
| Adjustment Create | AdjustmentCreate | Create [Adjustments](adjustments.html) and [TP Procedure Discounts](procedurefinancial.html). |  | X | AdjustmentDate |
| Adjustment Edit | AdjustmentEdit | Edit adjustments. | X | X | AdjustmentDate |
| Adjustment Edit Zero Amount | AdjustmentEditZero | Allow users without the Adjustment Edit or Adjustment Create permissions to create or delete adjustments with a zero amount and today's date as the *Entry Date*. |  |  |  |
...
| TP Procedure Delete | ProcDelete | Delete non-completed procedures and edit attached [Procedure Notes](procedurenotes.html) and [Procedure Group Note](procgroupnote.html). | X |  | DateEntryC |
| Procedure Note (full) | ProcedureNoteFull | Make changes to any procedure note. |  |  |  |
...
| Edit Note on Completed Procedure | ProcCompleteNote | Edit a note on a completed procedure.   Unchecking this permission also unchecks 'Edit EO or EC Procedures'. | X |  |  |
...
| **Web Applications** |  |  |  |  |  |
| Mobile Web/OD Mobile | MobileWeb | Allow or block a user from accessing the [Mobile Web Feature](../site/mobileweb.html) or [ODMobile](opendentalmobile.html). |  |  |  |
