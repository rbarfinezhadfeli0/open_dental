...
Alternatively, for user specific emails, in the Main Menu, click File, User Email Address. When accessed this way, only the user-specific email displays. No other users are able to see emails sent to the address.
...
This section displays when the Gmail is selected as the *Email Authentication* option..
...
This section displays when the Microsoft is selected as the *Email Authentication* option.
...
* **Email address of sender**: This email address is the display name. Not recognized by all email providers. Appears in the From address instead of the username (full email address), though the username credentials is still be used.
  + Do not use this field when utilizing Microsoft Authorization. Doing so may cause an error when sending emails.
  + When sending encrypted email, this field is ignored. (e.g., If *Sparkly Dental <bob@gmail.com>* the recipient sees *Sparkly Dental* as the sender instead of *bob@gmail.com*.)
...
* **Delete**: Delete the selected email address.
