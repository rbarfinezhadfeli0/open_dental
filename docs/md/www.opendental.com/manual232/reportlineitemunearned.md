...
* **Prov**: Only appears if *Show provider column is checked*Provider assigned to paysplit. This will be blank if no provider is assigned to the unearned income.
* **Amount**: Amount of the paysplit. This will differ from the payment amount if the payment includes multiple paysplits.