...
Alternatively, double-click an existing benefit from the Benefit Information grid to edit.

To see examples of other benefit scenarios that are known to work, see [Other Benefits](otherbenefits.html) or [Frequency Limitations](insfrequencylimitations.html). Adding benefits scenarios that are not listed on the Other Benefits or Frequency Limitations pages may be informational only and not affect insurance calculations.
...
* One of the following options can be used to apply the benefit.
  + **Category or** : Applies to [Insurance Category](inscatssetup.html). Used for Other Benefits.
...
* **Type:** Some types affect [Insurance Remaining Calculations](insremainingcalc.html); others are informational only.
  ...
  + CoPayment: Informational only. To be functional, copays should be added into a [Copay Fee Schedule](feeschedules.html).
