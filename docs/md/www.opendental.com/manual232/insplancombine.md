...
* Verify that employer, group #, group name, plan details, plan notes, and benefits (e.g., percenages, annual max, etc.) are all the same.
...
* Verify which plan has the most recent and accurate information. When combining, only one insurnace plan's details are kept. Other benefits marked as a patient override are not be lost when combining plans.
...
6. A warning appears. Click **OK** to proceeed with combing the insurance plans. Click **Cancel** to stop the process.
