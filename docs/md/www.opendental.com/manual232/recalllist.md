...
The interactive Recall List grid, shows patients due for a recall and their recall information. Customize which columns show in [Display Fields](displayfields.html), Recall List.
...
2. Configure the *Recall Types*, *Send Email Messages Automatically To* and *Send Text Messages Automatically To* options from [Web Sched Recall](webschedrecallsetup.html), but do not sign up for the eService.
