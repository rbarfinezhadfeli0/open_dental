...
| ccNumberMaskedWithExp | Available in Payment Plans. Shows the patient credit card on file as a masked number, with the associated expiration date.  (e.g., XXXXXXXXXXXX5555 Exp 12/23) |
...
| patient.provider | Usually patient's primary provider. For [Referral Slips](sheetsreferralslips.html) this is the Referring Provider. |
...
| statementURL | Include a URL patients can use to launch the Patient Portal sign-in window (e.g., http://www.patientviewer.com/statement/CyVue8). Patients must have Patient Portal credentials. After logging in, a PDF copy of the statement is displayed. If [Online Payments](paymentportalsetup.html) are enabled, patients can access the [Payment Portal](portalpaymentsbypatient.html) to make payments. |
