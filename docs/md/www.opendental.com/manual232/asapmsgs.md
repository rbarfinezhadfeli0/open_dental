...
* Checked: Prompt the user to send a text when Web Sched ASAP is enabled and a patient is on the ASAP list. Displays patients whose appointment will fit within the available timeslot.
...
Note: Message types that support HTML formatting will open the [HTML Email](emailhtml.html) editor. Raw HTML is not supported for Recall messages.
