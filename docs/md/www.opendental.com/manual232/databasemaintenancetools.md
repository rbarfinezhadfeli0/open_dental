...
Typically each tool takes a long time to run but only needs to be run once.
...
**Pay Plan Payments**: Detaches patient payments that have been applied to [Insurance Payment Plans](paymentplanins.html), and insurance payments attached to [Patient Payment Plans](paymentplanpatient.html).
...
**Update in progress on computer**: Manually clear the Update in Progress preference. This should be used when an update is complete but workstations are not able to log in due to the *Update in progress* error.