...
When opened from the Reports menu, this window is non-modal. When opened from Standard Reports, this window is modal.
...
* Checked: Query results exceeding column width will be wrapped onto a new line making the row wider.
* Unchecked: Query results exceeding column width will be cut off.

## Query Results

**Title:** The title of the query in the text box, if one exists. When running a query from Query Favorites, this box will prefill with the title saved to the favorite. Users can type into this box to label the query before printing.

**Query Results**: By default, the results will display in human-readable format. Columns vary based on the query being run. If a column header is prefaced with a $, the cells are always formatted as a dollar amount.
...
* **Right-click options**:
  ...
  + Go To Patient: (Only available for queries with a column of PatNum) The patient of the selected row will be selected in Open Dental.
...
* **Export:** Save the file as a tab-delimited txt or xls file to use in another program (e.g. Microsoft Word or Excel). Typical uses include letter merge, spreadsheet analysis, or financial imports. Set the default folder for exported queries in [Data Paths Setup](paths.html) (OpenDentalExports).
...
* [Custom Queries](queryoverview.html): For a fee, Open Dental Support can create a custom query.
