...
If the Treatment Plan [Sheet Def Defaults](sheetdefaults.html) includes the *SigPractice* field, when [Signing a Saved Treatment Plan](treatmentplansign.html) users will be prompted for both prompted for both a patient and a *Patient/Guardian* and *Office Signature*.
