...
![](images/securityLogOn.gif)

To protect data and prevent misuse, every user should be required to log on with a unique user name and password, then log off when they leave the workstation. See [Security](security.html) to set up user profiles and assign permissions.
...
Alternatively, if the [Global Security Setting](securitysettings.html), *Manually Enter Log On Credentials* is enabled, the Log On window will appear as shown below. Users must manually enter their user name to log in. The user list and filter will not be shown.
...
* After five consecutive failed attempts to log on, a user will temporarily be locked out of the account for five minutes or until a user with Security Admin privileges manually unlocks the account. See [Unlock Account](securityunlockaccount.html).
