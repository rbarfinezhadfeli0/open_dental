...
* **Autosave to Images**: Only appears when an [Definitions: Image Categories](definitionsimagecat.html) has usage set to *Autosave Forms*. Check to automatically save a copy of the sheet to the Imaging Module when clicking **OK**. Checked by default if the specific sheet is marked as *Autosave* in [Sheet Def Properties](sheetproperties.html).
