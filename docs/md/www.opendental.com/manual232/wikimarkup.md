...
The user edits the wiki markup and has no direct control over the actual html that results. Only the markup defined below is allowed. Any other html tags are not allowed, and the user will be forced to change them before the page can be saved. Most importantly, no attributes will be allowed other than those described. So no inline styles or any other typical html structures.
