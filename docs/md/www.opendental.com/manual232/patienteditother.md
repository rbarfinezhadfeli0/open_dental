...
* If the *Mask patient Social Security Numbers* preference is enabled, the social security number will mask the first 5 digits.
...
1. When a procedure is charted, regardless of status and there are no other completed procedures and the Date of First Visit is blank, then we update the Date of First Visit to match the appointment date.
2. When charting a procedure and the procedure status becomes Completed, the Date of First Visit is populated by the date of the completed procedure.
3. If there is an appointment the Date of First Visit is populated by the day but if the appointment is deleted and there are no completed procedures then we clear the Date of First Visit.
4. If an appointment is flagged as a New Patient Appointment (no past appointments and no completed procedures) and there are no completed procedures, the Date of First Visit is changed when the appointment moves to match the new appointment date.

Typically, the Date of First Visit is today's date but these are times in which the date can be a date other than today:

1. Adding a Repeating charge for a future charge date will change the Date of First Visit to the first recurring charge date.
2. Scheduling an appointment for a future or past day will change the Date of First Visit to match the day selected in the Appointment Module.
3. Setting a procedure complete in the Chart Module will use the date of the oldest procedure date.
4. Adding procedures through Appointment Types will set the Date of First Visit to the date of the appointment they're attached to.
5. Adding a procedure using Quick Add buttons will set the Date of First Visit to the date of the appointment they're attached to.
