...
In general, if you run into issues with timezones, first update Open Dental to the latest stable version, then contact Open Dental Support for input.

## Middle tier

[Middle Tier](middletier.html) is not typically an issue with timezones. The MySQL server time may be used when logging some date or time stamps (e.g., Audit Trail); this may differ from the middle tier server.
...
If you are using a [Remote Access](remote.html) app to access an Open Dental database, you may notice that your workstation is using the time zone of the database you are remotely accessing, rather than your local time. This will affect features in Open Dental such as the horizontal time line in the [Appointments Module](appointments.html).
