...
+ For Enterprise users, payment allocation is also affected by the *Enforce Valid Paysplits* setting in [Enterprise Setup](enterprisesetup.html) .
...
* Authorize charges for select patients with a [Payment Plan](paymentplanpatient.html) or [Repeating Charges](repeatingcharges.html). See [Authorize Recurring Charges](recurringchargesauthorize.html).
...
If a patient made a payment prior to the CC Recurring Charges tool being run, check the *Apply to Recurring Charge* box in the Payment window to manually apply the payment to an upcoming Recurring Charge to prevent the patient's credit card from being charged again. See Recurring Charge section in [Payment](payment.html) page for more information
