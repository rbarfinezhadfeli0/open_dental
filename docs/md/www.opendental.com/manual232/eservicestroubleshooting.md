...
* An *eConnector Down* [Alert](alerts.html).
...
5. Check your firewall settings. See [eConnector Firewall](allowapplication.html).
...
To view the Signup Portal, WebView2 must be installed. If WebView2 is not installed, a popup will display asking the user to download the installation. This only needs to be done once on each workstation.