...
![](images/messagingButtonSetup.gif)
...
1. Select the computer(s) this button will be set up on (an individual computer or All).
   * Select All if the button will be the same for all computers (e.g., Op 1, Op 2, Op 3, Pt Ready, Ph Asst).
   * Select one or multiple computers if the button is only relevant to certain computers (e.g., Hygiene ops only, Op 1 only, etc.).
2. Double-click a button in the Buttons area.
3. Enter button criteria, then click OK.
   ![](images/messagingButtonDrOp1.gif)
   ...
   * **Synch Icon** (optional): Enter a cell number (1-9) that lights up when the corresponding messaging button is clicked. 1-9 refers to one of the cells in Open Dental's program icon. ![](images/messagingButtonIcon.gif) The cells are numbered 1-9, left to right, starting with the top row and working down.
...
If the Synch Icon doesn't work, it may be due to your operating system. In Windows 7, 8 and 10 you may be able to correct this by right clicking on the taskbar and selecting properties (or taskbar settings). Look for the *Always Combine* option and change it to *Never Combine* or *Combine When Taskbar Is Full*.