...
| AutoNoteQuickNoteEdit | Created [Auto Notes](autonotes.html) and [QuickPaste Notes](quickpastenotessetup.html). |
...
| InsPlanChangeSubsc | Changed the subscriber on the Edit Insurance Plan window or use the [Move Subscribers](subscribersmove.html) tool. |
...
| PaymentCreate | Entered patient payment, created an [Income Transfer](incometransfer.html), or a Payment Portal payment was processed. |
| PaymentEdit | Edited patient payment. |
| PayPlanChargeDateEdit | Viewed a [Payment Plan](paymentplanpatient.html), [Dynamic Payment Plan](paymentplandynamic.html), or [Insurance Payment Plan](paymentplanins.html) charge on plans with APR. Logs an entry if the user has the *Pay Plan Charge Date Edit* security permission. |
| PayPlanChargeEdit | Edited or deleted a Dynamic Payment Plan or Patient Payment Plan charge |
| PayPlanEdit | Created, edited, closed, or deleted a Patient Payment Plan, Insurance Payment Plan, or Dynamic Payment Plan. |
...
| SheetEdit | * Edited sheets attached to patients and change the patient a sheet is attached to. * Tracks when web forms are retrieved and attached to patients ([Web Forms Retrieve](webformsretrieve.html)). * Lists information imported from a sheet ([Import Patient Forms and Medical Histories](patientformsimport.html)). |
