# File: ./www.opendental.com/manual/chart.html
...
<li><b>eRx:</b> Create an electronic prescription. If using DoseSpot, opens the <a href="dosespotuse.html">DoseSpot eRx / Prescription</a> interface. If the logged-on user is a Proxy user, a prompt displays to select a provider before launching the DoseSpot interface. If using EnsoraeRx (formerly NewCrop eRx), opens the <a href="newcrop.html">Ensora eRX / Prescription</a> interface. If eRx is not enabled for the practice, an error displays. Click the dropdown to view additional options. <ul>
