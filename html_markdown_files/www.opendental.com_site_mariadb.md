# File: ./www.opendental.com/site/mariadb.html
...
<body onload="BodyLoaded('mariadb','databasemanagementsystems','advancedtopics','documentation')">
	<nav class="LeftTree">
...
<li>MariaDB Server Variables: There are some users who do not use the Open Dental Installer (Trial Version) to install MariaDB. We only support MariaDB 10.11 in our installer. Any other versions of MySQL or MariaDB installed outside the Trial Version are not supported by Open Dental and may cause problems with the database. In addition, MariaDB must be installed using the myISAM default engine, not InnoDb or other storage engines (e.g., Aria). If the installer provided by Open Dental is not used, extra server variables may appear as part of the installation. </li>
...
<li>If using Windows 8.1 or Windows Server 2012 the following Windows update must be installed: <a href="https://support.microsoft.com/en-us/topic/update-for-universal-c-runtime-in-windows-c0514201-7fe6-95a3-b0a5-287930f3560c">KB2999226</a></li>
<li>MySQL 5.5, MySQL 5.6, or MariaDB 10.5 must be installed and currently functional.</li>
<li>Adequate storage space to create two full backups of the \mysql\data\ folder, plus an additional 220MBs.</li>
<li>Open Dental version 24.3 or greater is required for MariaDB 10.11.</li>
</ul>
...
<li>Ensure Open Dental is closed on all workstations. On the server, log into Open Dental and go to Tools, Misc Tools, <a href="../autoLogin.aspx%EF%B9%96ReturnUrl=%EA%A4%B7help253%EA%A4%B7shutdown.html">Shutdown All Workstations</a>, <b>Shutdown All</b>.</li>
...
<h2>Troubleshooting</h2><p>When launching Open Dental from a workstation, error "Object cannot be cast from DBNull to other types" may occur if the upgrade wizard was run prior to updating each instance of Open Dental. To resolve, run the setup.exe file found in the <a href="../autoLogin.aspx%EF%B9%96ReturnUrl=%EA%A4%B7help253%EA%A4%B7atozfolder.html">A to Z Folder</a>.</p>
